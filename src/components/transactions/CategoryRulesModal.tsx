...
import type { CategoryRule, CategoryType, Transaction, TransactionType } from "../../types/finance";
...
export const CategoryRulesModal: React.FC<{
  ...
  /** 고를 목록을 가리는 값입니다 — 규칙 자체에는 방향이 저장되지 않습니다. */
  const [draftDirection, setDraftDirection] = useState<TransactionType>("EXPENSE");
  ...
  const draftForm = (
    <div className="p-3 rounded-2xl bg-white border border-emerald-300 space-y-2.5 shadow-xs">
      ...
      <div>
        ...
        {/*
          **규칙에는 방향 칸이 없습니다** — 고른 카테고리가 이미 방향을 말합니다
          (`식비` 는 지출, `급여` 는 수입). 여기 토글은 **고를 목록을 가리는**
          것이고, 방향이 맞지 않는 규칙은 적용 단계에서 걸러집니다(§6.4).
        */}
        <div className="grid grid-cols-2 gap-1 p-0.5 bg-slate-100 rounded-lg mb-1.5">
          {([
            ["EXPENSE", "지출"],
            ["INCOME", "수입"],
            ...
            <button
              ...
              type="button"
              onClick={() => {
                if (value === draftDirection) return;
                setDraftDirection(value);
                setDraftCategory(value === "INCOME" ? "기타수입" : "식비");
              }}
