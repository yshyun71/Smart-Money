...
export const UserSecurityModal: React.FC<{
  ...
  const { user, registeredUser, hasPin, logout, lockApp, setPinCode } = useAuth();
  ...
  const handleSavePin = (e: React.FormEvent) => {
    ...
    setPinCode(newPin);
    setPinChangeMsg("간편 비밀번호가 성공적으로 변경되었습니다.");
        ...
        <div className="space-y-2">
          ...
          {!isChangingPin ? (
            <button
              ...
              className="w-full p-3 rounded-2xl bg-white border border-slate-200 hover:border-slate-300 text-left flex items-center justify-between transition touch-manipulation cursor-pointer"
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-xs font-bold text-slate-900">
                    {hasPin ? "간편 비밀번호 (6자리 PIN) 변경" : "간편 비밀번호 (6자리 PIN) 등록"}
                  </div>
            ...
            <form onSubmit={handleSavePin} className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
              <div className="flex items-center justify-between text-xs font-bold text-slate-900">
                <span>새 6자리 PIN 설정</span>
                <button
                  type="button"
                  onClick={() => setIsChangingPin(false)}
                  className="text-slate-400 hover:text-slate-600 text-[11px] p-1 cursor-pointer"
                >
                  취소
                </button>
              </div>
