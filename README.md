...
| **가계부** | 전체 거래 내역 검색·필터 |
| **카드·계좌** | 카드·계좌 등록, **CSV로 내역 가져오기**, 카드별·계좌별 조회와 수정/삭제/추가, 백업/복원 |
...
1. **파일 선택** — 어느 카드·계좌의 내역인지 고르고 CSV를 올립니다
...
**AI 서버가 혼잡할 때(503 UNAVAILABLE)** 는 지수 백오프로 최대 3회까지 자동 재시도하고, 대기 중임을 버튼에 표시합니다. 그래도 실패한 묶음이 있으면 **성공한 분류는 그대로 반영하고 실패한 항목만 선택 상태로 남겨** 두므로, 잠시 후 버튼을 다시 누르면 남은 것만 처리됩니다. 키 오류·네트워크 단절·사용량 한도처럼 기다려도 해결되지 않는 경우에는 즉시 중단합니다.

엑셀(.xlsx)은 지원하지 않습니다. 엑셀에서 **다른 이름으로 저장 → CSV**로 바꾼 뒤 올려주세요.
...
│   │   ├── csvImport.ts   CSV 디코딩·파싱·열 인식·중복 키
...
**시트를 겹쳐 띄울 때는 `createPortal`로 `<body>`에 올리세요.** 기기 프레임(`position: relative`)이 쌓임 맥락을 만들기 때문에, 그 안에서 z-index를 올려도 `<body>`에 붙은 시트 위로 올라갈 수 없습니다. 거래 수정 창이 계좌 내역 목록 뒤에 숨었던 원인입니다. 현재 계층은 계좌 내역 9998 · CSV 가져오기 9999 · 거래 추가/수정 10000입니다.
