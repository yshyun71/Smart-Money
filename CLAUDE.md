...
**이번 달 청구분 열 이름**: 카드사마다 `이번달 결제금액`(KB) · `이번달 납부금액`(신한) · `이번 달 입금하실 금액`(롯데) · `청구금액`으로 갈립니다. 모두 **지출** 열 후보입니다. 특히 롯데의 `입금하실`은 "입금"이 들어 있어 수입 열로 잡히기 쉬운데, 그러면 한 달 치 사용액이 통째로 수입이 됩니다 — `입금하실`은 입금 후보에서 제외합니다.
