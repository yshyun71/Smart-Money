...
npm test         # 회귀 세트 3종 — 명세서·카드연결·일괄적용 (15절)
...
- **비밀번호는 글자로 받지 않습니다.** 수정 화면의 `간편비밀번호 등록·변경` 버튼이 `PinSetupModal`을 띄우고, 거기서 키패드로 두 번 눌러 확인합니다. 이름·연락처와 **따로 저장**되므로 그 화면에서 바꾸면 `[변경 확인]`을 누르지 않아도 적용됩니다.
...
```bash
npm test     # statements + cardlink + spread
