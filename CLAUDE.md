...
- 카드 내역 화면에는 **잔액·수입 합계가 없습니다.** `이용 합계` 하나만 표시하고, 그 값은 **지출 − 차감·환불**입니다. 카드는 한 덩어리 금액을 청구하고 청구할인·환불은 그 금액을 **줄이는** 것이라, 지출만 더하면 명세서의 소계와 어긋납니다(우리카드에서 34,000원 차이가 났습니다). 차감이 있으면 `이용 … − 차감·환불 …`을 작은 글씨로 함께 보여 줍니다. **계좌는 다릅니다** — 거기서 수입은 들어온 돈이지 줄어든 청구액이 아니므로 지출과 수입을 따로 씁니다.
