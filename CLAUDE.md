...
**PICK의 계좌는 어디서 열었느냐로 정해집니다.** 카드·계좌 메인의 `엑셀·CSV 가져오기`는 첫 계좌로, **카드/계좌 내역 화면의 `엑셀·CSV` 버튼은 그 계좌로** 시작합니다(`defaultAccountId`). 사용자가 모달 안에서 바꾼 선택은 열려 있는 동안 그대로 둡니다. 14.8을 함께 보세요.
