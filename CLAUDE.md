...
- **값의 출처를 남깁니다** — `income_source`·`fixed_source`가 `ACTUALS`(실적에서 불러옴) 또는 `USER`(직접 적음)이고, 칸마다 `실적 반영` / `직접 입력` / `수정 중` 배지로 보입니다. 잔액의 `balance_source`(§8)와 같은 취지입니다: 숫자만 남기면 어디서 온 값인지 나중에 알 수 없고, 그 둘은 신뢰도가 다릅니다. 저축액은 실적에서 나올 값이 아니므로 늘 `USER`입니다.
...
- 세 가지 방식 중 하나를 고릅니다. `AMOUNT`(금액) · `INCOME_RATIO`(수입의 %) · `SPARE_RATIO`(가용 변동비의 %).
...
`tests/budget.test.ts`는 **예산 기준과 고정비 가이드**를 지킵니다(11.5): 가용 변동비 계산, 세 방식의 배분, 초과를 막지 않고 알릴 것, 규칙 없는 카테고리를 0으로 만들지 않을 것, 카테고리별 월평균과 추이, **진행 중인 달을 평균에서 뺄 것**, 가이드↔기준 환산, 고정비보다 낮은 한도 집어내기.
