...
- 그 기본값은 `guessBillingMonth(fileName, 내역들)`가 정합니다.
  1. **파일 이름에 적힌 연월이 우선**이고, **그 연월이 곧 결제월**입니다(더하지 않습니다) — `2026년3월 이용대금명세서(신한카드).xls`, `신한카드_202603_명세서.csv`, `202607_usage.csv`. 8자리 숫자(`20260803`)는 날짜이므로 연월로 쓰지 않습니다.
...
| 결제월 추정 | 파일 이름 10종, 이름이 없을 때 이용월+1, 연말 넘김, 할부 제외 |
