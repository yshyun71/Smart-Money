...
- 그래서 `findTable`은 **머리글 후보를 전부 시도하고, 그 아래 줄들이 실제로 "날짜 + 금액"으로 읽히는 개수가 가장 많은 후보를 고릅니다.** 힌트 단어 점수는 동점일 때의 보조 기준입니다. 읽히는 줄이 하나도 없으면(= 내역이 비어 있는 명세서) 가장 거래 표다운 머리글을 골라 사용자가 고칠 수 있게 합니다. 줄 수 제한은 없습니다.
...
**이번 달 청구분 열 이름**: 카드사마다 `이번달 결제금액`(KB) · `이번달 납부금액`(신한) · `청구금액`으로 갈립니다. 셋 다 지출 열 후보입니다.
