...
- 그 기본값은 `guessBillingMonth(fileName, 내역들)`가 정합니다.
  1. **파일 이름에 적힌 연월이 우선**이고, **그 연월이 곧 결제월**입니다(더하지 않습니다) — `2026년3월 이용대금명세서(신한카드).xls`, `신한카드_202603_명세서.csv`, `202607_usage.csv`. 읽는 차례는 `YYYY년 M월` → `YYYYMM`(구분자 있어도 됨) → `YYMM`입니다.
     - **`YYMM`(두 자리 연도)**: 롯데가 이렇게 씁니다 — `이용대금명세서_2608(신용.체크)_20260915091506.xls`의 결제월은 **2026-08**입니다. 숫자가 **정확히 네 자리**이고 앞 두 자리가 `20`~`39`일 때만 인정합니다.
     - **자릿수가 곧 안전장치입니다.** `20260803`(8자리)은 날짜, `20260915091506`(14자리)은 **내려받은 일시**로, 결제월과 아무 상관이 없습니다. 네 자리만 본다는 규칙이 이 둘을 통째로 걸러냅니다. 반대로 `2026`은 네 자리지만 `26`월이 없어 스스로 탈락합니다.
...
| 결제월 추정 | 파일 이름 17종(`YYMM`·생성일시 포함), 이름이 없을 때 이용월+1, 연말 넘김, 할부 제외 |
