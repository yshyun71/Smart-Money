...
- **합계 줄**: 대부분의 카드사는 합계 줄의 가맹점 칸을 비워 두므로 "가맹점이 없으면 내역이 아니다"로 걸러졌습니다. **삼성은 그 칸에 합계를 씁니다** — 결제월을 지정하면 날짜까지 얻어 185,200원짜리 이용 내역으로 등록됐습니다(바로 위 네 건의 합). `isTotalLabel`이 **`합계`·`소계`·`총계`·`누계`로 끝나면**(뒤에 `45건`이 붙어도) 합계로 봅니다.
  → **"포함"이 아니라 "끝"입니다.** 삼성은 그 구역 이름을 그대로 쓰고 뒤에 붙이므로 `일부결제금액이월약정(리볼빙) 일시불합계`처럼 길어집니다. 한국어는 핵심 명사가 뒤에 오니 끝으로 판단하면 이런 긴 이름도 잡히고, **`종합계좌이체`처럼 가운데에 `합계`가 든 진짜 거래는 살아남습니다.** 포함으로 바꾸면 그 거래가 조용히 사라집니다.
