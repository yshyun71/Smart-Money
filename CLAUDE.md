...
**수수료 열**(`수수료`·`이자`)은 따로 매칭합니다. **원금이 비어 있는 줄에서만** 금액으로 씁니다 — 롯데의 `크레딧케어 990`처럼 수수료만 있는 줄이 한 건으로 잡혀야 하고, KB처럼 원금과 수수료가 같은 줄에 있는 형식에서 둘을 더하면 별도 줄의 무이자 환급분과 이중 계산됩니다.
...
| KB국민카드 | 두 줄 머리글 병합, 회차가 금액이 되지 않을 것, 소계·합계·무이자 환급 줄 제외 |
