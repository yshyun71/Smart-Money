...
- **합계 줄**: 대부분의 카드사는 합계 줄의 가맹점 칸을 비워 두므로 "가맹점이 없으면 내역이 아니다"로 걸러졌습니다. **삼성은 그 칸에 `할부합계`라고 씁니다** — 결제월을 지정하면 날짜까지 얻어 185,200원짜리 이용 내역으로 등록됐습니다(바로 위 네 건의 합). `isTotalLabel`이 **칸 전체가 그 말일 때만** 합계로 봅니다(`합계`·`소계`·`총계`·`누계`, 뒤에 `45건` 붙는 것 포함). `종합계좌이체`처럼 이름 안에 든 글자는 가맹점으로 남깁니다.
...
| 삼성카드 | 가맹점 칸에 적힌 `할부합계` 제외, 이용금액이 아닌 `원금`, 8자리 이용일, 합계 일치 |
