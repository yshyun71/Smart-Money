...
npm test         # 회귀 세트 4종 — 명세서·카드연결·일괄적용·표시형식 (15절)
...
- 앱이 계산하는 것은 **`가용 변동비 예산 = 수입 − 고정비 − 저축`**(0 미만이면 0) 하나뿐이고, 그것이 `AI 스마트 예산 자동 분배`(50/30/20)의 재료입니다.
...
```bash
npm test     # statements + cardlink + spread + format
