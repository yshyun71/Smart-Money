...
- 설정 메뉴: `연동된 카드 및 계좌관리` / **`사용자 관리`** / `AI 등록` / `'스마트 머니' 전용 앱 설치`(§13.2) / **`기타 설정`**(**데이터 점검** — §17.7, **백업 알림 기간** — §12.12, 되돌리기 보관 기간과 최근 작업 — §4.9). **비밀번호 변경은 설정 메뉴에 없습니다** — 사용자 관리 안에 있습니다. 한 사람의 이름·연락처·비밀번호를 한자리에서 다루는 편이 맞고, 같은 일을 두 곳에 두면 한쪽만 고쳐집니다.
