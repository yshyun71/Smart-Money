...
- **한 사용자만 옮기는 길은 따로 있습니다**(§4.10). 전체 백업과 섞지 않습니다 — 이쪽은 기기를 바꾸는 단위이고, 그쪽은 사람을 옮기는 단위입니다.
...
### 4.10 사용자 한 명만 옮기기 (`services/userTransfer.ts`)

전체 백업은 **기기를 바꾸는** 단위입니다(§4.6). "내 것만 새 기기로 옮기고 싶다"에는 쓸 수 없습니다 — 그 기기에 있던 다른 사람의 가계부가 함께 사라지기 때문입니다. §4.6이 그때를 위해 적어 둔 방법이 이것이고, 그 방법 그대로 만들었습니다.

**성립하는 까닭**: 데이터가 전부 `user_id`로 닫혀 있고 **교차 참조가 사용자 경계를 넘지 않습니다** — 카드대금의 `linked_account_id`, 카드의 `payment_account_id`, 규칙의 `account_id`가 모두 같은 사람의 계좌를 가리킵니다. 한 사람을 들어내도 다른 사람 쪽에 끊어지는 실이 없습니다.

**담는 것**은 `PER_USER_TABLES` 아홉 테이블과 `users` 행입니다. **`repository.deleteUser`와 같은 목록이어야 합니다** — 한쪽에만 테이블을 더하면 지울 때는 남고 옮길 때는 빠지는 식으로 갈립니다(§5의 규칙 그대로). 회귀 세트가 `sqlite_master`를 훑어 **`user_id`를 가진 테이블이 목록에 다 있는지** 확인합니다.

- **id를 새로 매기지 않습니다.** `budget_configs`의 `income_excluded`·`fixed_excluded`·`savings_excluded`는 **거래 id가 든 JSON 배열**입니다(§11.4) — "이 15건은 실적에서 뺐다"는 사람의 판단이 거기 있습니다. id를 바꾸면 그 판단이 **오류 없이 조용히** 사라집니다(`sumActuals`가 모르는 id를 무시하므로). 그래서 파일의 id를 그대로 쓰고, **그 대가로 파일의 id가 누구를 갈아 끼울지를 정합니다.**
- **먼저 지우고 나서 넣습니다**(`replaceUser`), 그리고 **한 배치**입니다(§4.8). 반쯤 적용되면 주인 없는 데이터나 데이터 없는 주인이 남고, 그것은 되돌릴 방법이 없는 손상입니다. 두 번 넣어도 늘지 않습니다(§14.3 멱등).
- **`undo_log`도 함께 비웁니다.** 오래 빠져 있던 구멍입니다 — 남겨 두면 그 id를 다시 쓰는 사람에게 **이전 사람의 행 사본**이 딸려 가고, 되돌리기는 역연산이 아니라 담아 둔 행을 그대로 다시 쓰는 방식이라(§4.9) 누르는 순간 지워진 거래가 새 가계부에 되살아납니다. `deleteUser`에도 함께 넣었습니다.
- **복원 뒤에는 반드시 로그아웃합니다.** 지금 세션의 id가 갈아 끼워졌을 수 있고, 그러면 모든 조회가 `WHERE user_id = '없는 id'`가 되어 **0건**이 됩니다 — §4.6·§14.8에서 두 번 겪은 "데이터가 전부 사라진 것처럼 보이는" 그 증상입니다.
- **되돌릴 수 없습니다.** 한 사람의 전체를 담기에는 되돌리기 저장소가 감당할 양이 아닙니다. 확인 창이 그 사실과 **건수**를 함께 말합니다(§12.8) — `계좌·카드 7개와 내역 1,420건이 지워집니다`.

**거절하는 경우**(`checkImport`) — 판정은 순수 함수이고 화면은 까닭을 그대로 보여 줍니다.

| 까닭 | 왜 |
| --- | --- |
| `NOT_A_USER_FILE` | 전체 백업(`.db`·`.smbk`)을 여기 넣은 경우. 그쪽 길을 안내합니다 |
| `NEWER_SCHEMA` | 더 새로운 버전에서 만든 파일. **사다리는 위로만 놓여 있습니다**(§4.3) — 짐작해 여는 것은 지어내는 일입니다(§17.1) |
| `NAME_TAKEN` | 이름은 로그인 화면에서 사람을 고르는 값입니다(§5). 둘이면 누구인지 고를 수 없습니다 |
| `NO_USER`·`EMPTY` | 옮길 것이 없습니다 |

**`user_primary`를 조심하세요.** 첫 사용자가 쓰던 **고정 id**라 기기마다 다른 사람이 그것을 갖고 있을 수 있습니다(오늘 만들어지는 id는 `u-${시각}-${난수}`라 부딪히지 않습니다). 그때는 남의 가계부를 지우는 일이므로 `nameChanges`가 참이 되고, 화면이 **누가 지워지는지**를 더 분명히 말합니다.

**옛 버전 파일은 사다리를 태웁니다**(`alignUserFile`). 전체 백업은 `PRAGMA user_version`을 달고 다녀 열 때 알아서 따라잡지만(§4.1) 이 파일에는 그 장치가 없습니다. 그래서 **빈 메모리 DB를 파일의 버전까지만 마이그레이션해** 행을 넣고, `migrate()`로 현재까지 끌어올린 뒤 도로 꺼냅니다. 컬럼 추가뿐 아니라 **값을 옮기는 마이그레이션**(v6·v7의 카테고리 분리, v14의 저축, v18의 방향별 분리)이 그대로 적용됩니다.
→ **변환을 따로 만들지 않는 것이 요점입니다.** 옮기기 전용 규칙을 쓰면 §4.3의 사다리와 어긋나는 날이 오고, 그러면 같은 데이터가 경로에 따라 다르게 분류됩니다(§17.6).
→ 실제 v11 백업(1,420건)으로 확인했습니다: v11 → v18로 올린 뒤 건수 그대로, 남은 `기타 금융` 0건.

**검증을 걸지 않습니다**(§17.7의 예외). 복원은 "파일이 진실"이라는 약속이고, 여기서 줄을 걸러 내면 파일에 있는 것이 조용히 들어오지 않습니다. 대신 들인 뒤에 데이터 점검(§17.7)으로 무엇이 이상한지 **보고**합니다.

**따라가지 않는 것**: `undo_log`(임시 저장소 — 옮길 이유가 없습니다), 그리고 **localStorage에 있는 것 전부** — AI 키(§11.1), 기억된 명세서 형식(§7.4), 숨긴 알림·백업 알림 기간·마지막 백업 시각(§12.12). 전체 백업도 마찬가지지만, 사용자별 복원은 "이 사람 것을 전부 옮겼다"고 기대하기 쉬워 **화면이 그 사실을 적습니다.**

파일은 **JSON**입니다(전체 백업이 `.db`인 것과 다릅니다 — 담는 것이 행뿐이라 데이터베이스 파일일 이유가 없습니다). 확장자는 `.smuser`이고, 암호를 걸면 전체 백업과 **같은 방식**으로 잠급니다(§4.5). 암호가 걸렸는지는 **확장자가 아니라 머리의 표식**으로 판별합니다. 크기는 작지 않습니다 — 1,420건이 604KB로, 전체 `.db`(618KB)와 비슷합니다.
