...
// ---------------------------------------------------------------------------
{
  ...
  check("회차가 메모에 실림", r.drafts[0]?.memo === "할부 9/10", r.drafts[0]?.memo);
...
// ---------------------------------------------------------------------------
section("삼성카드 — 가맹점 칸에 적힌 할부합계, 이자면제 혜택금액");
// ---------------------------------------------------------------------------
{
  const samsung = [
    "할부,,,,,,,,,,,,,",
    ...
    '20260601,본 인 289,주식회사 지마켓,"241,240",,이자면제,"-1,189",3,3,"80,400",0,,0,0',
    ',,할부합계,,,,,,,"185,200",0,,0,"125,200"',
  ].join("\n");

  const r = read(samsung, { fallbackDate: "2026-08-01" });

  check("이용금액이 아니라 원금이 청구 열", r.mapping.withdrawal === 9, r.mapping);
  check("8자리 이용일", r.drafts[0]?.date === "2025-12-08", r.drafts[0]?.date);
  check("원금이 청구액", like(r.drafts[0], { merchant: "(주)엔에스쇼핑", amount: 16_000 }), r.drafts[0]);

  /*
    다른 카드사는 합계 줄의 가맹점 칸을 비워 두지만 삼성은 거기에 "할부합계"를
    적는다. 결제월을 지정하면 그 줄이 날짜를 얻어 185,200원짜리 이용 내역으로
    등록됐다 — 바로 위 네 건을 더한 값이다.
  */
  check("할부합계는 내역이 아님", r.drafts.length === 4, r.drafts.map((d) => d.merchant));
  check(
    "합계 줄이 섞이지 않음",
  ...
  check(
    "합계가 명세서의 할부합계와 같음",
    r.drafts.reduce((sum, d) => sum + d.amount, 0) === 185_200,
    r.drafts.reduce((sum, d) => sum + d.amount, 0)
  );

  // 이자면제 혜택금액(음수)은 이미 원금에 반영돼 있어 금액이 되지 않는다
  check("혜택금액은 금액 열이 아님", r.mapping.deposit === -1, r.mapping);
  check("이자/수수료 0은 더해도 그대로", r.drafts[1]?.amount === 18_200, r.drafts[1]);

  // 합계 판정은 칸 전체가 그 말일 때만 — 가맹점 이름 안에 든 글자는 아니다
  ...
  */
  for (const label of [
    ...
    "총계",
    "이용합계",
    // 우리 — 가운데에 들어가고, 뒤에 한글이 아니라 괄호·부호·숫자가 온다
    "소계(양승현)",
    "청구합계-국민은행 357******155",
    "카드소계",
    "청구결제번호소계",
...
// ---------------------------------------------------------------------------
section("우리카드 — 이름 중간에 든 소계·청구합계, 음수 차감");
// ---------------------------------------------------------------------------
{
  const woori = [
    "이용대금명세서 상세 내역,,,,,,,,,,,,,",
    ...
    '07.20,신용/본인,0097,국내일시불부결제금액이월약정(리볼빙),배스킨라빈스 구로디지털단지역점,"15,300",,1,"15,300",0,0,0,0,0',
    '08.15,신용/본인,0097,국내일시불부결제금액이월약정(리볼빙),네이버파이낸셜,"17,400",,1,"17,400",0,0,0,0,0',
    '08.17,신용/본인,0097,차감,차감-[청구할인] 청호나이스 우리카드II 장기할부 할,"-10,000",,0,"-10,000",0,0,0,0,0',
    '08.17,신용/본인,0097,차감,차감-[청구할인] 청호나이스 우리카드II 장기할부 프,"-7,000",,0,"-7,000",0,0,0,0,0',
    ',신용/본인,0097,카드소계,소계(양승현),,,0,"814,623",0,0,0,"969,984",0',
    ',신용/본인,0097,청구결제번호소계,청구합계-국민은행 357******155,,,0,"814,623",0,0,0,"969,984",0',
  ].join("\n");

  const r = read(woori, { fallbackDate: "2026-09-01" });

  check("두 줄 머리글 병합", r.table.headers[8] === "당월결제하실금액 원금", r.table.headers[8]);
  check("원금이 청구 열", r.mapping.withdrawal === 8, r.mapping);
  check("카드구분이 아니라 매출구분이 메모", r.mapping.memo === 3, r.mapping);

  /*
    우리카드는 합계를 이름 가운데에 씁니다 — "소계(양승현)", "청구합계-국민은행
    357******155". 끝나는 말로만 보던 규칙은 이 둘을 놓쳤고, 결제월을 지정하면
    날짜까지 얻어 814,623원짜리 이용 내역 두 건으로 등록됐습니다.
  */
  check("합계 두 줄 제외 — 5건", r.drafts.length === 5, r.drafts.map((d) => d.merchant));
  check(
    "소계·청구합계가 섞이지 않음",
    !r.drafts.some((d) => /소계|합계/.test(d.merchant)),
    r.drafts.map((d) => d.merchant)
  );
  check("814,623원이 내역으로 들어오지 않음", !r.drafts.some((d) => d.amount === 814_623), r.drafts);

  check("할부는 이번 회차 원금", like(r.drafts[0], { merchant: "청호나이스", amount: 53_888 }), r.drafts[0]);
  check("회차가 기간과 함께 메모에", /18\/36/.test(r.drafts[0]?.memo || ""), r.drafts[0]?.memo);

  /*
    차감-[청구할인]은 청구액에서 빼는 돈입니다. 지출로 읽으면 그 달에 10,000원을
    더하게 되고, 월 합계가 어떤 출금과도 맞지 않아 카드대금 자동 연결이 깨집니다.
  */
  const deducted = r.drafts.filter((d) => d.merchant.startsWith("차감"));
  check("차감 2건", deducted.length === 2, deducted);
  check("차감은 지출이 아님", deducted.every((d) => d.type === "INCOME"), deducted);
  check("차감 금액", deducted.map((d) => d.amount).join(",") === "10000,7000", deducted);

  const signed = r.drafts.reduce(
    (sum, d) => sum + (d.type === "INCOME" ? -d.amount : d.amount),
    0
  );
  check("차감이 빠진 합계", signed === 53_888 + 15_300 + 17_400 - 10_000 - 7_000, signed);
}
...
// ---------------------------------------------------------------------------
{
  const named: [string, string | null][] = [
    ...
    // KB — 8자리는 결제월 + 발송일. 9월 명세서를 9월 3일에 보낸다
    ["이용대금명세서_20260903.xlsx", "2026-09"],
    // 삼성 — 같은 8자리인데 뒤가 결제일. 앞의 연월이 결제월인 것은 같다
    ["samsungcard_20260826.xlsx", "2026-08"],
    ["이용대금명세서_20261231.xlsx", "2026-12"],
    ["명세서_20261301.xls", null], // 13월은 없다
    ["명세서_20260932.xls", null], // 32일도 없다
    // 신한 — 이름 앞에 결제월을 적는다
    ["2026년9월 이용대금명세서(신한카드).xls", "2026-09"],
    // 결제월을 적어 둔 이름이 발송일보다 앞선다
    ["이용대금명세서_2608_20260903.xlsx", "2026-08"],
...
// ---------------------------------------------------------------------------
section("할부 회차 — 다섯 카드사가 한 모양으로 (§7.6)");
// ---------------------------------------------------------------------------
{
  /*
    회차는 **중복 판정의 키**입니다. 할부는 매달 같은 날짜·가맹점·금액으로 다시
    청구되므로 회차만이 달을 구분합니다 — 없으면 다음 달 명세서의 할부 건이
    지난달 것으로 걸러집니다.

    그런데 카드사마다 적는 자리가 다르고, **같은 이름이 다른 것을 담습니다**:
    롯데의 `할부` 는 총 개월(10)이고 KB 의 `할부` 는 회차까지 붙은 쌍(`10/10`)
    입니다. 이름만 보고 뜻을 정할 수 없어, 값에 `/` 가 있으면 그대로 씁니다.
  */
  const markerOf = (lines: string[]): string | undefined => {
    const table = parseDelimited(lines.join("\n"));
    const mapping = autoDetectMapping(table.headers, table.rows);
    return buildDrafts(table, mapping).drafts[0]?.memo;
  };

  check(
    "KB — 할부 한 칸에 10/10",
    markerOf([
      "이용일자,이용카드,구분,이용하신 가맹점,이용금액,할부,원금,수수료",
      '25.10.19,JCB097,할부,롯데홈쇼핑-방송에서만1,"279,000",10/10,"25,947",433',
    ]) === "할부 10/10",
    markerOf([
      "이용일자,이용카드,구분,이용하신 가맹점,이용금액,할부,원금,수수료",
      '25.10.19,JCB097,할부,롯데홈쇼핑-방송에서만1,"279,000",10/10,"25,947",433',
  ...
  check(
    "우리 — 할부개월 + 회차",
    markerOf([
      "이용일자,카드구분,이용카드,매출구분,이용가맹점(은행)명,이용금액,할부개월,회차,원금,혜택금액,환율,수수료,결제후잔액,할부가격",
      '03.12,신용/본인,0097,국내할부,청호나이스,"1,940,000",36,17,"53,888",0,0,0,"1,023,872",0',
    ]) === "국내할부 17/36"
  );
  ...
  check(
    "롯데 — 회차 + 할부",
    markerOf([
      "이용일,이용카드,이용가맹점,이용총액,회차,할부,원금,수수료,적립예정,적립예정,이용혜택,혜택금액,결제 후 잔액",
      '2026.04.11,본인LOCA LIKIT 1.2,에스케이스토아,"136,800원",5,10,"13,600",,,,무이자할부,"1,322","68,000"',
    ]) === "5/10"
  );
  ...
  check(
    "신한 — 할부기간 + 회차",
    markerOf([
      "이용일,이용카드,이용가맹점,이용금액,할부기간,회차,원금,수수료(이자),적용 구분,결제 후 잔액,포인트적립율",
      '2026.03.04,본인353,홈플러스방학점,"41,940",10,3,"41,940",0,할부,0,0',
    ]) === "할부 3/10"
  );

  /*
    **총 개월만 있고 회차가 없으면 아무것도 적지 않습니다.** 그 값은 매달 같아서
    달을 구분하지 못하고, 회차인 척 적으면 다음 달 같은 할부가 지난달 것과 같은
    키를 갖습니다.
  */
  check(
    "회차가 없으면 비워 둡니다",
    markerOf([
      "이용일,이용가맹점,이용금액,할부개월,원금",
      '2026.04.11,에스케이스토아,"136,800",10,"13,600"',
    ]) === "",
    markerOf([
      "이용일,이용가맹점,이용금액,할부개월,원금",
      '2026.04.11,에스케이스토아,"136,800",10,"13,600"',
  ...
  /* 돈이 든 열을 개월로 착각하지 않습니다 — `총할부금액`·`할부가격` */
  check(
    "할부가격은 개월이 아닙니다",
    markerOf([
      "이용일,이용가맹점,이용금액,할부가격,회차,원금",
      '2026.04.11,에스케이스토아,"136,800","1,368,000",5,"13,600"',
    ]) === "5회차"
  );

  /* 회차만 있으면 예전처럼 `N회차` 입니다 — 그것만으로도 달은 갈립니다 */
  check(
    "기간을 모르면 회차만",
    markerOf([
      "이용일,이용가맹점,이용금액,회차,원금",
      '2026.04.11,에스케이스토아,"136,800",5,"13,600"',
    ]) === "5회차"
  );
}
